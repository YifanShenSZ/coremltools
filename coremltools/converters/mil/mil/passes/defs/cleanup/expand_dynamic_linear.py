        ...
        if is_weight_static:
            if is_bias_static:
                ...
                block.replace_uses_of_var_after_op(
                    ...
                    new_var=add,
                    # need force_replace since when we ambitiously use linear,
                    # weight and bias must be descendant of const thus unreplaceable
                    force_replace=True,
                )
            ...
            # static bias, try avoid add if all zero
            if is_bias_static:
                # if no bias provided, default to 0, can avoid
                # if bias provided, need to inspect its value
                if op.bias is not None:
                    if op.bias.op.op_type == "const":
                        is_nonzero_bias = np.any(op.bias.val != 0)
                    else:
                        is_nonzero_bias = np.any(op.bias.op.materialized_val_inference() != 0)
                    if is_nonzero_bias:
