...
class TorchConverter:
    ...
    def convert_const(self) -> None:
        def _add_const(name, val):
            ...
            # TODO (rdar://107718371): support uint8 quantization for TorchScript
        ...
        for name, val in self.graph.params.items():
            if self.context.frontend == TorchFrontend.TORCHSCRIPT:
                ...
                with mb.scope(
                    ...
                    _add_const(name, val)
            # TODO(rdar://117541129): Add scope to support traceability for EXIR constants
            else:
                _add_const(name, val)
    ...
    def convert(self) -> Program:
        ...
        with Function(ssa_func_inputs, opset_version=self.opset_version) as ssa_func:
            ...
            # Right now we only check for torchscript source
            # The one for EXIR is tracked by rdar://117541129 ([Infra][Pytorch] Support traceability from torch EXIR -> coreml)
            if self.context.frontend == TorchFrontend.TORCHSCRIPT:
